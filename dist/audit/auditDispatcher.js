...
exports.AuditDispatcher = exports.AuditDeliveryError = void 0;
...
exports.AuditDeliveryError = AuditDeliveryError;
...
const cloneAuditValue = (value, depth, seen, budget) => {
    budget.remaining -= 1;
    ...
    for (const [key, child] of entries) {
        if (BLOCKED_KEYS.has(key))
            continue;
        try {
            cloned[key.slice(0, 128)] = cloneAuditValue(child, depth + 1, seen, budget);
        }
        catch {
...
const detachedMetadata = (metadata) => {
    const cloned = cloneAuditValue(metadata ?? {}, 0, new WeakSet(), { remaining: MAX_METADATA_NODES });
    return deepFreeze(cloned);
};
const optionalSafeString = (value, label, maximumLength) => {
    if (value === undefined)
        return undefined;
    if (typeof value !== "string" ||
        value.length === 0 ||
        ...
        throw new Error(`${label} must contain between 1 and ${maximumLength} safe characters`);
    }
    return value;
};
const detachedActor = (actor, label) => {
...
class AuditDispatcher {
    constructor(config = {}) {
        ...
        if (config.onDeliveryError !== undefined &&
    ...
    async emit(input) {
        ...
        const sessionId = optionalSafeString(input.sessionId, "audit.sessionId", 256);
        const tenantId = optionalSafeString(input.tenantId, "audit.tenantId", 256);
        const correlationId = optionalSafeString(input.correlationId, "audit.correlationId", 128);
        ...
        const deliveries = await Promise.allSettled(this.sinks.map((sink) => Promise.resolve().then(() => sink.write(event))));
        ...
        if (this.onDeliveryError) {
            await Promise.allSettled(failures.map((error) => Promise.resolve().then(() => this.onDeliveryError(error, event))));
        }
...
exports.AuditDispatcher = AuditDispatcher;
//# sourceMappingURL=auditDispatcher.js.map