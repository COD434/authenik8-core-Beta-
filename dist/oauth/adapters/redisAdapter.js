...
const emailKey = (email) => `${PREFIX}:email:${normalizeEmail(email)}`;
const providerKey = (provider, providerId) => `${PREFIX}:provider:${provider}:${providerId}`;
...
const createRedisIdentityAdapter = (redis) => {
    const lock = new lockHelper_1.RedisLock(redis);
    ...
    return {
        ...
        async linkProvider(userId, provider, providerId) {
            ...
            try {
                ...
                const hasProvider = existingUser.providers.some((entry) => entry.provider === provider && entry.providerId === providerId);
                if (!hasProvider) {
