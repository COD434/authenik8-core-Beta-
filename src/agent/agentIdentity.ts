...
export class AgentIdentityService implements AgentIdentityApi {
  ...
  constructor(options: AgentIdentityServiceOptions) {
    if (
      ...
      throw new AgentIdentityError(
        "AGENT_INVALID",
        "Agent identity requires valid registry and delegation callbacks",
      );
    }
    if (
    ...
    this.tokenExpirySeconds = normalizeTokenLifetime(
      options.config.tokenExpiry ?? DEFAULT_AGENT_TOKEN_EXPIRY,
      "agent.tokenExpiry",
      60,
    ...
    this.sessions = new SessionStore(
      options.redisClient,
      this.scoped(AGENT_SESSION_NAMESPACE),
    );
  ...
  async issueDelegatedToken(
    ...
    try {
      ...
      const delegatedUserId = assertDelegatedIdentity(
        user.userId,
        "Delegated userId",
      );
      const delegatedSessionId = assertDelegatedIdentity(
        user.sessionId,
        "Delegated sessionId",
      );
      ...
      const sessionActive = await this.hasHumanSession(
        delegatedUserId,
        delegatedSessionId,
      );
      if (!sessionActive) {
  ...
  async verifyToken(token: string): Promise<AgentTokenPayload | null> {
    try {
      const unverifiedUse = (await decodeBoundedJwt(token)).tokenUse;
      ...
      if (
        ...
        return this.rejectVerifiedAgent(
          payload,
          "delegating_session_inactive",
        );
      }
  ...
  private claimsAreValid(payload: AgentTokenPayload): boolean {
    try {
      ...
      return (
        payload.sub === `user:${delegatedUserId}` &&
  ...
  private riskPrincipal(payload: AgentTokenPayload) {
    return {
      kind: "agent" as const,
      id: payload.agentId,
  ...
  private async rejectVerifiedAgent(
  ...
  private async auditAgentRejection(
    agentId: string,
    error: unknown,
  ): Promise<void> {
    let actor:
    ...
    await this.audit?.emit({
      ...
      metadata: {
        reason:
          error instanceof AgentIdentityError ? error.code : "issuance_failed",
      },
    });
  }
}
